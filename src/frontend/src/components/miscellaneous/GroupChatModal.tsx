...
const GroupChatModal: FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const handleDelete = (delUser: IUser) => {
        ...
        <ModalContent>
          ...
          <ModalBody display="flex" flexDir="column" alignItems="center">
            ...
            {loading ? (
              ...
              searchResult
                ?.slice(0, 4)
                .map((user: IUser) => <UserListItem key={user._id} user={user} handleFunction={() => handleGroup(user)} />)
            )}
          </ModalBody>
          <ModalFooter>
