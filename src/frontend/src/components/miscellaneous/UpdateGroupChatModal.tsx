...
const UpdateGroupChatModal: FC<{ fetchMessages: () => void; fetchAgain: boolean; setFetchAgain: (a: boolean) => void }> = ({
  fetchMessages,
  fetchAgain,
  setFetchAgain,
}) => {
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      ...
      const { data } = await callAxios("get", `user?search=${search}`, {}, config);
      console.log(data);
      setLoading(false);
  ...
  const handleAddUser = async (user1: IUser) => {
  ...
  const handleRemove = async (user1: IUser) => {
        ...
        <ModalContent>
          ...
          <ModalBody display="flex" flexDir="column" alignItems="center">
            ...
            {loading ? (
              ...
              searchResult?.map((user: IUser) => <UserListItem key={user?._id} user={user} handleFunction={() => handleAddUser(user)} />)
            )}
          </ModalBody>
          <ModalFooter>
